...
//    var arrow : Arrow!
    ...
    fileprivate  var valueArray = [Float]()
    ...
    public var valueDataArray = [Float]() {
        didSet{
            self.valueArray = self.valueDataArray
        }
    }
...
//        arrow = Arrow(origin: CGPoint(x: center.x - arrowSize/2,y: center.y - arrowSize/2),size: arrowSize  )
        arrowImageView = UIImageView(frame: CGRect(x: center.x - arrowSize/2, y: center.y - arrowSize/2, width:15 , height: 15))
...
extension DropDown: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if cell == nil {
            ...
            let lblValue = UILabel(frame: CGRect(x: 0, y: 0, width: 80, height: 40))
            lblValue.sizeToFit()
            lblValue.text = String.init(format: "$%.2f", arguments: [self.valueArray[indexPath.row]])
            lblValue.font = UIFont(name: "BrandonGrotesque-Regular", size: 14)
            lblValue.textColor = .white
            lblValue.textAlignment = .right
            lblValue.tag = 100
            lblValue.translatesAutoresizingMaskIntoConstraints = false
            let horizontalConstraint = lblValue.trailingAnchor.constraint(equalTo: (cell?.contentView.trailingAnchor)!, constant: -16)
            let verticalConstraint = lblValue.centerYAnchor.constraint(equalTo: (cell?.contentView.centerYAnchor)!)
            cell?.contentView.addSubview(lblValue)
            cell?.contentView.addConstraints([horizontalConstraint, verticalConstraint])
        }
        ...
        cell?.textLabel?.font = UIFont(name: "BrandonGrotesque-Regular", size: 14)
